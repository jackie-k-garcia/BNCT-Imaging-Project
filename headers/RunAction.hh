...
class RunAction : public G4UserRunAction
{
  public:
    ...
    inline void fillPerHit(G4int, G4int, G4int, G4double);
...
inline void RunAction::fillPerHit(G4int IDinX, G4int IDinY, G4int IDinM, G4double PixelEnergy)
{
  ...
  // BEGINNING OF EVENT: Event Number, Multiplicity
  if (PixelEnergy == -2)
  {
    IDinX = 0;
    IDinY = 0;
  ...
  else if(PixelEnergy == -1)
  {
    ...
    outputDataFile.open('pixel_grid_hit_data_unformated');

    // Fill the output data file.
    outputDataFile << "Source Photons: " << EventNumber << G4endl
                   << "Detected:       " << Detected << G4endl
    ...
    /*** Reseting the variables at the end of the current hit ***/
    TotalPixelEnergy = 0;
    Multiplicity = 0;
  }
